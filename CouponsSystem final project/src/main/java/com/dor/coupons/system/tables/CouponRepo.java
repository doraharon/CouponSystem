...
public interface CouponRepo extends CrudRepository<Coupon, Long>{
	...
	@Query("SELECT COUP FROM COUPON AS COUP WHERE COUP.endDate >= :coupon_endDate AND COUP.amount > 0")
	Collection<Coupon> getCoupons (@Param("coupon_endDate")Date coupon_endDate);
}